...
const AddRisksForm = ({ trafficLineData, params: { stationId, schoolId }, files }) => {
    ...
    const [fixedQuestionAnswer, setFixedQuestionAnswer] = useState('لا'); // Default to "no"
        ...
        const fetchQuestions = async () => {
            try {
                ...
                const formattedQuestions = data.map(q => ({
                    ...q,
                    answer: 'غير مجاب عليها' // Default answer to "unanswered"
                }));
    ...
    const handleAnswerChange = useCallback((id, answer) => {
        setQuestions(prevQuestions =>
            prevQuestions.map(q =>
                q.id === id ? { ...q, answer: answer === 'لا ينطبق' ? 'لا' : answer } : q
            )
        );
    }, []);

    const handleFixedQuestionChange = useCallback((answer) => {
        setFixedQuestionAnswer(answer);
    }, []);
    ...
    const questionCard = useMemo(() => (
        questions.map(({ id, question, translatedQuestion, answer }, idx) => {
            const isHidden = fixedQuestionAnswer === 'لا' && idx < 9;
            const questionNumber = fixedQuestionAnswer === 'نعم' ? idx + 2 : idx - 7;

            return (
                <div key={id} className={`flex flex-col items-start gap-3 py-2 border-b ${isHidden ? 'hidden' : ''}`}>
                    <span className="flex-1">
                        {questionNumber}- {language === 'ar' ? question : translatedQuestion}
                    </span>
                    ...
                    {answer !== 'غير مجاب عليها' && (
                        <span className={`text-sm font-semibold mt-2 ${answer === 'لا' ? 'text-red-700' : answer === 'نعم' ? 'text-green-700' : 'text-[#111]'}`}>
                            {t('the answer')}: {t(answer)}
                        </span>
                    )}
                </div>
            );
        })
    ), [questions, handleAnswerChange, fixedQuestionAnswer, language, t]);
    ...
    return (
        <div className="min-h-[calc(100vh-148px)] flex flex-col justify-center items-center">
            <div className="w-full p-4 max-w-xl my-12 max-h-[650px] border overflow-y-scroll bg-white rounded-lg shadow-md flex flex-col gap-5">
                <div className="flex flex-col items-start gap-3 py-2 border-b">
                    <span className="flex-1">1- الميكروباص في همم ولا لا؟؟</span>
                    <div className="flex items-center gap-2">
                        <Button size='sm' className={`${fixedQuestionAnswer === 'نعم' && 'bg-green-800 hover:bg-green-700'}`} onClick={() => handleFixedQuestionChange('نعم')}>{t('نعم')}</Button>
                        <Button size='sm' className={`${fixedQuestionAnswer === 'لا' && 'bg-red-800 hover:bg-red-700'}`} onClick={() => handleFixedQuestionChange('لا')}>{t('لا')}</Button>
                    </div>
                </div>
                {questionCard}
